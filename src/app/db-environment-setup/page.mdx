...
```javascript
// config/validation.js const Joi =
require("joi");
...
async function getDatabaseConfig() {
  if (process.env.NODE_ENV === "production") {
    ...
    const credentials =
      await secretsManager.getDatabaseCredentials("production");
...
class VaultClient {
  ...
  async getDatabaseCredentials(environment) {
    try {
      const result = await this.client.read(
        `secret/data/myapp/${environment}/database`
      );
  ...
  async getTemporaryDatabaseCredentials(environment, ttl = "1h") {
    try {
      const result = await this.client.read(
        `database/creds/myapp-${environment}-role`
      );
      return {
...
async function seedWithFactories() {
  ...
  for (let i = 0; i < 200; i++) {
    ...
    const orderProducts = faker.helpers.arrayElements(createdProducts, {
      min: 1,
      max: 5,
    });
...
// migrations/1640000000000-CreateUsers.ts
